...
use crate::{
    Application, RUSTIC_APP, // [FIXED] 修正为正确的变量名
...
pub fn write_snapshots_as_sql<W: Write>(
    ...
    for (_, snapshots) in groups {
        for snap in snapshots {
            let id = snap.id.to_hex().to_string(); // [FIXED]
            writeln!(writer,
                     "INSERT OR REPLACE INTO snapshots VALUES ('{}', '{}', '{}', {}, '{}', '{}', '{}', {}, {}, {}, '{}', {}, '{}');",
                     id,
                     snap.time.to_string(),
                     sql_escape(&snap.program_version),
                     // [FIXED] 显式调用 to_string()
...
pub fn fill_table(snap: &SnapshotFile, mut add_entry: impl FnMut(&str, String)) {
    add_entry("Snapshot", snap.id.to_hex().to_string()); // [FIXED]
    if let Some(original) = snap.original {
        if original != snap.id {
            add_entry("Original ID", original.to_hex().to_string()); // [FIXED]
        }
    }
